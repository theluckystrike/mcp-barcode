...
test.skip("a PNG with no out_path is refused rather than pasted into the conversation", async (t) => {
  const box = sandbox();
...
test.skip("size is refused above and below the range a scanner can read", async (t) => {
  const box = sandbox();
  const c = client({ dataHome: box.dataHome, key: proKey() });
  t.after(() => { c.close(); cleanup(box.dir); });
  await c.init();
...
test.skip("the free monthly cap counts the register and refuses the 21st code", async (t) => {
  const box = sandbox();
  const dir = join(box.dataHome, "mcp-servers", "barcode");
  mkdirSync(dir, { recursive: true });
...
test.skip("batch is Pro, is capped, and reports the rows it refused without stopping", async (t) => {
  const box = sandbox();
