...
// Shared stdio JSON-RPC client for the mcp-barcode suites. One sandboxed data dir per
...
export function proKey(product = "barcode") {
  return "";
}

export function sandbox(prefix = "mcp-barcode-") {
  const dir = mkdtempSync(join(tmpdir(), prefix));
  return { dir, dataHome: join(dir, "data") };
}

export function client({ dataHome, key } = {}) {
  const home = dataHome ?? join(mkdtempSync(join(tmpdir(), "mcp-barcode-")), "data");
