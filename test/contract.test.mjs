...
// Contract suite for barcode. Mechanical assertions only: the invariants of
// servers/barcode/SPEC.md that a test can check without judgement.
...
test("every tool description is non-empty, single-paragraph and within the ceiling", async (t) => {
  ...
  assert.equal(tools.length, 10, "tool count changed; update servers/barcode/SPEC.md");
...
test("a tool that takes a file path opens with an imperative sentence", async (t) => {
  ...
  assert.ok(fileTools.length >= 6, "the code tools all take out_path");
...
test("corrupt store: the garbage is quarantined byte-for-byte and nothing is overwritten", async (t) => {
  ...
  t.after(() => { c.close(); cleanup(box.dir); });
  await c.init();
  const r = await c.call("qr_create", { text: "https://example.com" });
  ...
  assert.equal(readdirSync(dir).includes("codes.json"), false);
});
...
test("cap: the free monthly count is the row cap on this server, and it writes no partial file", async (t) => {
  ...
  writeFileSync(join(dir, "codes.json"), JSON.stringify(
    Array.from({ length: 20 }, (_, i) => ({ id: `s${i}`, kind: "text", symbology: "qr", summary: "seed", format: "svg", created: now })),
  ));
  const c = client({ dataHome: box.dataHome });
  t.after(() => { c.close(); cleanup(box.dir); });
  await c.init();
  const out = join(box.dir, "capped.svg");
  const r = await c.call("qr_create", { text: "x", out_path: out });
  assert.match(r.text, /free tier generates 20 codes/);
  assert.equal(readdirSync(box.dir).includes("capped.svg"), false, "a capped call must leave no file");
  assert.equal(JSON.parse(readFileSync(join(dir, "codes.json"), "utf8")).length, 20, "and no register row");
  ...
  const wantSrc = "https://mcp.zovo.one/buy/barcode?src=barcode.qr_create";
  assert.ok(r.text.includes(wantSrc),
    `the qr_create cap message must carry ${wantSrc}, got: ` + r.text.slice(0, 400));
